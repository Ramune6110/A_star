...
vector<vector<int>> A_star::calc_obstacle_map(vector<int> ox, vector<int> oy, const int min_ox, const int max_ox, const int min_oy, const int max_oy, float resolution, float robot_radius)
{
...
vector<Node> A_star::get_motion_model()
{
    return {Node(1, 0, 1),
            Node(0, 1, 1),
            Node(-1, 0, 1),
...
float A_star::calc_heristic(Node* n1, Node* n2)
{
    float w = 1.0;
    float d = w * sqrt(pow(n1 -> x - n2 -> x, 2) + pow(n1 -> y - n2 -> y, 2));
    return d;
}

bool A_star::verify_node(Node* node, vector<vector<int>> obmap, int min_ox, int max_ox, int min_oy, int max_oy)
{
...
vector<vector<float>> A_star::calc_final_path(Node * goal_node, float resolution)
{
    vector<float> rx;
    vector<float> ry;
    Node* node = goal_node;
...
void A_star::a_star_method(vector<float> ox_, vector<float> oy_)
{
    Node* start_node = new Node((int)round(sx / resolution), (int)(sy / resolution), 0.0);
    Node* goal_node  = new Node((int)round(gx / resolution), (int)(gy / resolution), 0.0);
    ...
    // path_cost
    vector<vector<float>> path_cost(xwidth, vector<float>(ywidth, numeric_limits<float>::max()));
    path_cost[start_node->x][start_node->y] = 0;
    ...
    while (true) {
        ...
        for (int i = 0; i < motion.size(); i++) {
            Node * new_node = new Node(node -> x + motion[i].x, node -> y + motion[i].y, path_cost[node -> x][node -> y] + motion[i].cost + calc_heristic(goal_node, node), node);
            ...
            if (path_cost[node->x][node->y] + motion[i].cost < path_cost[new_node->x][new_node->y]) {
                path_cost[new_node->x][new_node->y] = path_cost[node->x][node->y]+motion[i].cost; 
